...
class Post extends DbTemplate {
	...
	function __construct(){
		...
		// Set the labels
		$this->SetLabels(array('author_id'=>'Author:', 'category_id'=>'Category:'));
		...
		// Set the Display
		$this->SetDisplay(array('title', 'author_id', 'category_id', 'tags', 'body'));
	...
	public function Form($display='', $hidden=array(), $options=array(), $config=array(), $omit=array(), $multi=false){ 
		...
		$categories = $myCategory->GetList(array('category'),'category','DESC');
		$category_list = array();
		
		// Format the category list how we would like
		foreach($categories as $category_id=>$category)
			$category_list[$category_id] = $category['category'];
		
		// Add State Options
		$this->SetOption('category_id', $category_list, 'Please Select');
...
class Category extends DbTemplate {
	...
	function __construct(){
		...
		// Set the required
		$this->SetRequired(array('category'));
		
		// Set the Display
		$this->SetDisplay(array('category'));
	}
}
